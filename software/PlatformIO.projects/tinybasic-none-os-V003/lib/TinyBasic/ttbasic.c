...
//  -Add TIME, milli second timer
//  -Add SAVE, SAVE BOOT, LOAD for last 1K flash program
//
//
//
#include <stdlib.h>
...
void  isave() {
    ...
    flash_save(&listbuf[0]);            // プログラムをセーブ
    listbuf[SIZE_LIST-1] = 0;            // BOOT指定を戻す。

    putnum(1024-getsize(), 0);  
    if (boot==0x55)
      c_puts(" saved+boot!"); 
    else
      c_puts(" saved!"); 
...
void basic() {
  ...
  if( flash_boot() == 0x55)  {   // もし　BOOT指定が 0x55なら
      flash_load(&listbuf[0]);              // 領域番号０をロード
      listbuf[SIZE_LIST - 1] = 0;   // BOOT指定を戻す。
      irun();   // 実行する。
  }

  //起動メッセージ
  c_puts("\nTOYOSHIKI TINY BASIC"); //「TOYOSHIKI TINY BASIC」を表示
  newline(); //改行
