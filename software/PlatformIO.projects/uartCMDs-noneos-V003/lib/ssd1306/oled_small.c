...
extern const uint8_t fontRobotWidth[];
extern const uint8_t *fontRobotAddr[];
extern void OLED_plotRobot(uint8_t x, uint8_t y, char* s);

//jimmy.su add 128x64 128x32, select one!
//#define SSD128x64
#define SSD128x32
...
const uint8_t ssd1306_init_sequence [] = {	// Initialization Sequence
  ...
  #endif
  #ifdef SSD128x32
   0xA8, 0x1F,		// Set multiplex ratio (1 to 32) ... (height - 1)
  #endif
	...
	0x81, 0x7F,		// Set contrast control register - 0x01 to 0xFF - Default: 0x3F jimmy:0x7F got bolder character
  ...
  #ifdef SSD128x64
	 0x22, 0x00, 0x07	// Set Page Address (start,end) 0 - 63
  #endif
  #ifdef SSD128x32
   0x22, 0x00, 0x03,	// Set Page Address (start,end) 0 - 32
  #endif
...
//plot roboto fonts
void OLED_plotRobot(uint8_t x, uint8_t y, char* s) {
